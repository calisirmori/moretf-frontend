...
const MapEventsHeatMapKills: React.FC<MapEventsHeatMapProps> = ({
    events,
    mapName = "default",
    localPlayerId,
}) => {
    ...
    useEffect(() => {
        ...
        const killEvents = events.filter(
            (e) => e.eventType === "kill" && e.actingPlayerID === localPlayerId
        );
    ...
    return (
        <div className="w-full p-3 bg-light-100 dark:bg-warm-800 text-warm-800 dark:text-light-100 font-semibold rounded-md">
            <h3 className="text-sm text-warm-800 dark:text-light-100 font-semibold pb-1">Kills Heatmap</h3>
...
export default MapEventsHeatMapKills;
